...
def setup():
    ...
    environment_id = "lcls"
    app_name = "app-three"
    interface = K2EGInterface(environment_id, app_name)
...
def run_iteration(snd_model, interface, input_pvs):
    ...
    # TODO: send in a dict of the input names but mapped to PV somehow, so that we dont have to map back
    ...
    if abs(input_dict['energy'] - snd_model.input_variables['energy'].default_value) > energy_change_threshold:
        ...
        # TODO: which delay to use here? For now, we use the default value.
        snd_model.initialize_model(input_dict['energy'], snd_model.input_variables['delay'].default_value)
        # Set new default energy
        snd_model.input_variables['energy'].default_value = input_dict['energy']
        logger.info(f"New default energy is {snd_model.input_variables['energy'].default_value}.")
